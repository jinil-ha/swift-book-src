...
enum ParseType {
	case NONE, NUMBER, OPERATOR, ALPHABET, BRACKET_OPEN, BRACKET_CLOSE
	func isSafe( nextParse:ParseType ) -> Bool {
		switch self {
		case NONE, OPERATOR, BRACKET_OPEN:
			switch nextParse {
			case NUMBER, ALPHABET, BRACKET_OPEN:
				return true
...
class Parser {
	...
	func parse() -> [String]? {
		...
		for ch in self.string.characters {
			...
			case "a"..."z":
				if !lastParse.isSafe(ParseType.ALPHABET) {
					return nil
				}
				
				if lastParse == ParseType.ALPHABET {
					lastElement!.append(ch)
				} else {
					if lastParse != ParseType.NONE {
						result.append(lastElement!)
			...
			case "(":
				...
				if lastParse == ParseType.ALPHABET {
					switch lastElement! {
					case "sqrt","sin","cos","tan","exp","log":
						result.append(lastElement!)
						
					default:
						print("ERROR : cannot parsing string ", lastElement!)
						return nil
					}
				} else if lastParse != ParseType.NONE {
					result.append(lastElement!)
					NSLog("element : \(lastElement!)")
				}
			...
			default:
				print("ERROR : cannot parsing character '%@'", String(ch))
				return nil
			}
		}
